...
//      Заполняем символьный массив символами строки которую ввел пользователь и по ходу ловим знак операции
        ...
        if ((Character.isDigit(stable00.charAt(0)) & !Character.isDigit(string03.charAt(0))) || (!Character.isDigit(stable00.charAt(0)) & Character.isDigit(string03.charAt(0)))) {
            throw new IllegalArgumentException("римские и арабские не совмещать!!!!!!!!");
        }

        if (blacks.length > 2) {
            throw new IllegalArgumentException("не более двух значений!!!!!!!!!!");
        }
           ...
           // if ((number1 < 0 || number2 < 0) && (number1 > 10 || number2 > 10)) {
             if (number1 < 0 || number2 < 0){
                ...
                if (operation == '-' && number1 < number2) {
                    throw new IllegalArgumentException("В римском исчеслении нет отрицательных чисел ");
                } else if (operation == '-' && number1 == number2) {
                    throw new IllegalArgumentException("В римском исчеслении нет нуля");
                } else {
                    result = calculated(number1, number2, operation);
                    String resultRoman = convertNumToRoman(result);
                    System.out.println("---Результат для римских цифр----");
                    String asd = stable00 + " " + operation + " " + string03 + " = " + resultRoman;
            ...
            if ((number1 < 0 || number2 < 0) || (number1 > 10 || number2 > 10)) {
                throw new IllegalArgumentException("вводить только положительные числа от 1 до 10");
            } else {
                result = calculated(number1, number2, operation);
                System.out.println("--Результат для арабских цифр----");
                String asd = number1 + " " + operation + " " + number2 + " = " + result;
                return asd;
                //System.out.println(asd);
